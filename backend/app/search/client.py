...
def bulk_index_logs(client: OpenSearch, logs: List[Dict[str, Any]]) -> Dict:
    ...
    max_attempts = 5
    backoff_base = 2.0  # Increased from 1.0 for longer delays
    attempt = 0
    last_exc = None

    while attempt < max_attempts:
        try:
            # Use smaller initial chunk size to reduce memory pressure
            chunk_size = max(10, int(settings.batch_size / (attempt + 1)))
            success, errors = helpers.bulk(client, actions, chunk_size=chunk_size, raise_on_error=False)
            
            # Log detailed error information
            if errors:
                logger.error(f"Bulk indexing had {len(errors)} errors. First 3 errors:")
            ...
            last_exc = te
            # If 429 (circuit breaker or rate limit), apply backoff and retry with smaller chunks
            ...
            if status_code == 429:
                ...
                # Longer delay for circuit breaker to recover
                time.sleep(backoff)
                # Reduce chunk size more aggressively
                try_chunk = max(5, int(settings.batch_size / (2 ** (attempt + 1))))
                logger.info(f"Retrying with chunk_size={try_chunk}")
